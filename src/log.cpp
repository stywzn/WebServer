...
{
    // 1. 如果设置了max_queue_size，则设置为异步模式
    if (max_queue_size >= 1)
    {
        m_is_async = true;
    ...
    {
        // 情况B: 带有路径 logs/server
        strcpy(log_name, p + 1);
        
        // 【修正】使用 strncpy 限制长度，+1 是为了包含 '/'
        strncpy(dir_name, file_name, p - file_name + 1);
        dir_name[p - file_name + 1] = '\0'; // 【修正】手动加上结束符，防止乱码
